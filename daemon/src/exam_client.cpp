...
ExamClient::~ExamClient() {
    if (!channel_name_.empty() && event_queue_ != nullptr) {
        try {
            Event unregister_client_event =
                Event::create_unregister_client_event(channel_name_);
            event_queue_->post_event(unregister_client_event);
        } catch (...) {
        }
    }
